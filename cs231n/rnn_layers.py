...
def rnn_step_backward(dnext_h, cache):
    ...
    dfc = dnext_h * (1 - next_h**2) #shape(N, H)
    dx = dfc.dot(Wx.T)
    dWx = x.T.dot(dfc)
...
def rnn_backward(dh, cache):
    ...
    for i in range(T):
        if i is T-1:
            ...
            prev_h = h[:,T-i-2,:]
        next_h = h[:,T-i-1,:]
        cache_i = (x[:,T-i-1,:], prev_h, next_h, Wx, Wh, b)
        dx[:,T-i-1,:], dh_i, dWx_i, dWh_i, db_i = rnn_step_backward(dh[:,T-i-1,:]+dh_i, cache_i)
        dWh += dWh_i
        dWx += dWx_i
        db += db_i
        if i is T-1:
